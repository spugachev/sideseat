...
/// The complete inventory of positions where a resolver-authoritative feed would differ from today's.
///
/// Printed rather than asserted, because it exists to be **reviewed** before the projection changes.
/// The plan's step 2 was gated on byte-identity and that gate cannot be met; the honest replacement is a
/// reviewed inventory, and an inventory measured from the *first* differing index (as the first attempt
/// was) understates it - five fixtures where the truth is eight feed views, missing a category entirely.
#[test]
#[ignore = "measurement for step 2, run explicitly: cargo test feed_projection_inventory -- --ignored --nocapture"]
fn feed_projection_inventory() {
    let fixtures = discover_fixtures();
    if fixtures.is_empty() {
        eprintln!("inventory: no fixtures - skipping");
        return;
    }
    let pricing = PricingService::init_for_test().expect("offline pricing service");
    ...
    for (label, paths) in &fixtures {
        let rows = rows_for_mode(&pricing, paths, ExtractionMode::PerCarrier);
        crate::domain::sideml::feed::FEED_PROJECTION_DELTA.with(|d| d.borrow_mut().clear());
        let _ = build_golden(label, paths, &rows);
        let delta: Vec<String> =
            crate::domain::sideml::feed::FEED_PROJECTION_DELTA.with(|d| d.borrow().clone());
        if !delta.is_empty() {
            affected.push(label.clone());
            total += delta.len();
            eprintln!("== {label}: {} differing position(s)", delta.len());
            for line in delta.iter().take(12) {
                eprintln!("     {line}");
            }
        }
    }
    eprintln!(
        "\nfeed projection inventory: {total} positions across {} feed views: {:?}",
        affected.len(),
        affected
    );
}
