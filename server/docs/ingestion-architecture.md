...
| the frame arrives *inside* an ordered array that extraction fragments into per-index carriers | 5 — langgraph | **fixed**: the resolver re-groups the `llm.input_messages` family per span (both halves of the interning key canonicalised) and orders its **first-seen** members through lineage, history included — the framing block's own discipline, which is what neutralises a replay where the raw history gate could not be lifted. Broadening the same rule to Vercel's `ai.prompt` regressed a sequential two-step trace (`call1, result1, call2, result2` → `call1, call2, result1, result2`), so each further family needs its own measured pass |
| frame on the agent span, turn on the orchestrator span | 1 — `strands/swarm` | **closed as an information limit**, measured: Strands *rewrites* the question before the model sees it (`Context: User Request: …`, a different digest), so no identity links the surviving question to any request, and the wrapped copy is correctly filtered as a context echo. The event-stream generalisation of the array rule was implemented against it and reverted — thousands of forward no-op edges, no new cycles, zero repair, because the target does not exist as an orderable unit. Ordering the displayed question against the planner's frame would need content-similarity matching, which is the heuristic class this work exists to remove |
