...
| the frame arrives *inside* an ordered array that extraction fragments into per-index carriers (`llm.input_messages.N.message` interns as N carriers), so no sequence edge survives | 5 — langgraph | open, and review 22 established the repair is **two** changes, not one: canonicalisation (resolver-site only — extraction-site breaks the OI multimodal-enrichment lookup, dedup rank scopes, and misses persisted history; and `PayloadKey`'s `payload_root` must be canonicalised too, or `.0`/`.1` still intern apart) **plus** something for the history gate, because the generation's user copy is history and is dropped before lineage projection, so a one-member carrier draws no edge. A generic "let history into carrier sequences" is unsafe — that exclusion is what stopped ADK's replay imposing false global order. Predicted repair delta of canonicalisation alone: **zero** |
